...
final case class Scope(typeScope: TypeScope = Map.empty,
                       imports: CompilationUnits = null,
                       varScope: VariableScope = Map.empty,
                       filename: Filename = "Primitive")
{
...
final case class PreTypedModule(imports: CompilationUnits, classes: Map[TypeId, ModuleMember])(implicit val scope: Map[TypeId, ModuleMember], val filename: Filename)

final case class TypedModule(imports:CompilationUnits, classes: Map[TypeId, TModuleMember])(implicit val scope: Scope)

sealed abstract class TModuleMember(val name: TypeId)(implicit val scope: Scope)
...
final case class TCombinedArgs(formalArgs: TFormalArgs, args: TArgs)(implicit val scope: Scope)
final case class TArg(expr: Option[TExpr], ofType: Option[TOfType], assign: Option[TExpr])(implicit val scope: Scope)
...
final case class TBlock(contents:List[TBlockContent], catchBlock: Option[TBlock], alwaysBlock: Option[TBlock])(implicit val scope: Scope) extends TBlockContent
final case class TIs(list: List[TTypeClass])(implicit val scope: Scope)
final case class TDeclareMap(map: List[TPonyMap])(implicit val scope: Scope)
final case class TPonyMap(from:BodyContent, to: ID)(implicit val scope: Scope)

final case class TOfType(typeList: Set[TTypeElement])(implicit val scope: Scope)


final case class TTypeBody(body: Map[ID,TBodyContent])(implicit val scope: Scope)

sealed abstract class TBodyContent(val name: ID, val isAbstract: Boolean = false)(implicit val scope: Scope)
...
final case class TMethodContent(mode: TMode, id:ID, combinedArgs: TCombinedArgs)(implicit val scope: Scope)

final case class TExpr(unary: TUnary, operator: List[(Operator, TUnary)])(implicit val scope: Scope) {
...
final case class TCaseBlock(c: Option[TCaseSubBlock], block: TBlock)(implicit val scope: Scope)
abstract class TCaseSubBlock(implicit val scope: Scope)
...
final case class TCaseVar(expr: Option[TExpr], forVar: TForVar)(implicit val scope: Scope)
final case class TForVar(id: ID, ofType: TOfType)(implicit val scope: Scope)


sealed abstract class TLValue(implicit val scope: Scope)
...
sealed abstract class TUnary(unaryOps: List[UnaryOp])(implicit val scope: Scope)
...
final case class TCommand(first: TFirstCommand, second: Option[TSecondCommand])(implicit val scope: Scope)

sealed abstract class TFirstCommand {
  def extractOfType(implicit scope: Scope): TOfType = ???
}

final case class TCommandExpr(expr: TExpr)(implicit val scope: Scope) extends TFirstCommand
final case class TCommandArgs(args: List[TArg])(implicit val scope: Scope) extends TFirstCommand
